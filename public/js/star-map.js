...
async function showEntityNeighborhood(names) {
  ...
  // ⏱ in the graph opens a timeline scoped to whatever entities are currently in view (B → C).
  const openScopedTimeline = async (scopeSeeds) => {
    try {
      const [{ openTimeline }, tl] = await Promise.all([
        import("./timeline.js"),
        post("/api/library/timeline", { names: scopeSeeds }),
      ]);
      if (!tl || !tl.events || !tl.events.length) { alert(t("star_entityTimelineEmpty", { name: (scopeSeeds || []).join("、") })); return; }
      openTimeline(tl.events, { onOpenDoc: (docId) => { closeStarMap(); openLibraryDoc(docId); } });
    } catch (e) { console.error("entity timeline failed", e); }
  };
