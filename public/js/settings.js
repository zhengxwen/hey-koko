  ...
  localStorage.setItem(
    ...
    JSON.stringify({
      ...
      comfyParams: {
        ...
        scailMemory: dom.comfyParamScailMemory?.value || "",
...
function saveChatMessage(message) {
  ...
  if (message.generatedVideos && message.generatedVideos.length > 0) {
    ...
    // Per-clip metadata as parallel arrays (batch video-edit) → resend reconstructs
    // each clip. The scalar videoMime/videoName/… are gone; legacy ones are folded
    // into these arrays on load (migrateVideoFields).
...
export function loadSavedSettings() {
  ...
  if (savedSettings.comfyParams) {
    ...
    if (dom.comfyParamUpscaleTarget) dom.comfyParamUpscaleTarget.value = cp.upscaleTarget || "";
    ...
    // "" is the default (both optimisations), so an absent key correctly lands there
    // rather than on the path that OOMs on long clips.
    if (dom.comfyParamScailMemory) dom.comfyParamScailMemory.value = cp.scailMemory || "";
