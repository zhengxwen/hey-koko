...
#
# The .command extension is what makes this double-clickable in macOS Finder
# (Finder refuses to run a .sh that way). It is only a name — the file is an
# ordinary shell script, so on Linux `./start.command` runs exactly the same.
cd "$(dirname "$0")"
